...
US20            | Actualizar coordenadas de usuario    | **Como** desarrollador, **quiero** actualizar las coordenadas de un usuario a través del API  **para** mantener su ubicación actualizada en la base de datos. | ***Scenario 1***: Actualizar ubicación de usuario <br>**Dado** que el endpoint '/users/coordinates' esta disponible,<br> **Cuando** se envía una solicitud PUT<br> **Entonces** las coordenadas del usuario seleccionado por su id se actualiza en la base de datos. | EP04                    |
US21            | Actualizar coordenadas de vehiculo | **Como** desarrollador, **quiero** actualizar las coordenadas de un vehiculo a través del API  **para** mantener su ubicación actualizada en la base de datos. | ***Scenario 1***: Actualizar ubicación de vehiculo <br>**Dado** que el endpoint '/vehicles/coordinates' esta disponible,<br> **Cuando** se envía una solicitud PUT<br> **Entonces** la ubicación del vehiculo seleccionado por su id se actualiza en la base de datos. | EP04                    |
US22            | Distancia entre usuario y vehículo   | **Como** desarrollador, **quiero** encontrar la distancia y el tiempo entre el usuario y su vehículo través del API  **para** mantener la base de datos actualizada. | ***Scenario 1***: Calcular distancia entre usuario y su vehiculo <br>**Dado** que el endpoint '/tracking/distance' esta disponible,<br> **Cuando** se envía una solicitud GET<br> **Entonces** el usuario y su vehiculo correspondiente seleccionados por su id se obtiene la distancia en kilometros en que se encuentran estos. <br> ***Scenario 2***: Calcular tiempo entre usuario y su vehiculo <br>**Dado** que el endpoint '/tracking/distance' esta disponible,<br> **Cuando** se envía una solicitud GET<br> **Entonces** el usuario y su vehiculo correspondiente seleccionados por su id se obtiene el tiempo que hay entre estos. | EP04                    |
US23            | Crear notificaciones       | **Como** desarrollador, **quiero** crear notificaciones a través del API  **para** mantener informado al usuario respecto al estado de su vehículo y mantener la base de datos actualizada. | ***Scenario 1***: Notiicacion por sospechas <br>**Dado** que el endpoint '/notifications' esta disponible,<br> **Cuando** se envía una solicitud POST<br> **Entonces** el usuario seleccionado por su id recibirá una notificación si hay sospechas en su vehículo. <br> ***Scenario 2***: Notiicacion por distancia <br>**Dado** que el endpoint '/notifications/distance-alert' esta disponible,<br> **Cuando** se envía una solicitud POST y el usuario se encuentra a más de 2km de su vehículo<br> **Entonces** el usuario seleccionado por su id recibirá la notificación "Te encuentras lejos de tu vehículo". <br> ***Scenario 3***: Notiicacion por distancia <br>**Dado** que el endpoint '/notifications/distance-alert' esta disponible,<br> **Cuando** se envía una solicitud POST y el usuario se encuentra a menos de 2km de su vehículo<br> **Entonces** el usuario seleccionado por su id recibirá la notificación "Estas cerca de tu vehículo".| EP04                    |
US24            | Presentación de sitio web | **Como** visitante, **quiero** ver un resumen sobre lo que ofrece la aplicación móvil  **para** mantenerme informado. | ***Scenario 1***: Visualización de resumen <br>**Dado** que el visitante accede a nuestra landing page,<br> **Cuando** visualiza el sitio web<br> **Entonces** se siente satisfecho al ver que puede encontrar lo que quiera destacando puntos importantes. | EP05                    |
US25            | Información footer | **Como** visitante, **quiero** más información sobre la empresa en el footer **para** mantener despieto mi interés. | ***Scenario 1***: Información clave <br>**Dado** que el visitante se encuentra en lapágina web,<br> **Cuando** baje hasta el footer<br> **Entonces** podrá ver información adicional de la empresa como cookies y permisos legales. | EP05                    |
US26            | Navegación de información | **Como** visitante, **quiero** información de una sección en específico  **para** no perder tiempo. | ***Scenario 1***: Uso del navbar <br>**Dado** que el visitante se encuentra en la página web,<br> **Cuando** utilice el navbar<br> **Entonces** podrá navegar entre secciones con más facilidad. | EP05                    |
US27            | Sección de contacto | **Como**visitante, **quiero** comunicarme con un miembro de la empresa  **para** resolver dudas. | ***Scenario 1***: Soporte al cliente <br>**Dado** que el visitante se encuentre en la sección de contacto,<br> **Cuando** envía un correo, indicando sus credenciales<br> **Entonces** su duda, pregunta, consulta, seran contestadas. | EP05                    |
...
<br><br><center>![ImpactMap](assets/chapter02/impact-mapping.png)</center>
...
| 16      | US16          | Autenticación de usuario              | Como desarrollador, quiero visualizar los usuarios registrados en la aplicacion, para gestionar la base de datos de estos.                | 3                                |
| 17      | US17          | Crear vehículo         | Como desarrollador, quiero agregar un nuevo vehiculo a través del API para gestionar la base de datos de estos.                 | 3                                |
| 18      | US18          | Editar vehículo              | Como desarrollador, quiero editar y actualizar informacion de un vehiculo a través del API para mantener la base de datos actualizada.                  | 3                                |
| 19      | US19          | Eliminar vehículo        | Como desarrollador, quiero borrar un vehiculo a través del API para mantener la base de datos actualizada.               | 3                                |
| 20      | US20          | Actualizar coordenadas de usuario            | Como desarrollador, quiero actualizar las coordenadas de un usuario a través del API para mantener su ubicación actualizada en la base de datos.                  | 5                                |
| 21      | US21          | Actualizar coordenadas de vehículo            | Como desarrollador, quiero actualizar las coordenadas de un vehiculo a través del API para mantener su ubicación actualizada en la base de datos.                  | 5                                |
| 22      | US22          | Distancia entre usuario y vehículo                 | Como desarrollador, quiero encontrar la distancia y el tiempo entre el usuario y su vehículo través del API para mantener la base de datos actualizada.                  | 5                                |
| 23      | US23          | Crear notificaciones                 | Como desarrollador, quiero crear notificaciones a través del API para mantener informado al usuario respecto al estado de su vehículo y mantener la base de datos actualizada.                  | 5                                |
24            | Presentación de sitio web | Como visitante, quiero ver un resumen sobre lo que ofrece la aplicación móvil  para mantenerme informado. | 5                    |
25            | Información footer | Como visitante, quiero más información sobre la empresa en el footer para mantener despieto mi interés. | 3                    |
26            | Navegación de información | *Como visitante, quiero información de una sección en específico  para no perder tiempo. | 5                    |
27            | Sección de contacto | Como visitante, quiero comunicarme con un miembro de la empresa  para resolver dudas. | 3                    |